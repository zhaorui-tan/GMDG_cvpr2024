...
def main():
    ...
    if args.wandb_name:
        # if args.local_rank == 0:
         wandb.init(project='Res', name=args.wandb_name, config=args)
    ...
    # y_args.lr = 1e-3
    Y_net = network.get_net(args, criterion, criterion_aux)
    Y_net.cuda()
    Y_net = nn.DataParallel(Y_net)
    Y_optim, Y_scheduler = optimizer.get_optimizer(args, Y_net)
    # Y_net = torch.nn.SyncBatchNorm.convert_sync_batchnorm(Y_net)
    # Y_net = network.warp_network_in_dataparallel(Y_net, args.local_rank)
    # teacher_model = convert_net(Y_net, args)

    f_mean_encoders = None
    f_var_encoders = None
    # if args.teacher:
    #     f_mean_encoders = nn.ModuleList([MeanEncoder((args.bs_mult,1024 * 2, 48, 48))])
    #     f_var_encoders = nn.ModuleList([VarianceEncoder((args.bs_mult,1024 * 2, 48, 48))])
    #     net_list.append(f_mean_encoders)
    #     net_list.append(f_var_encoders)
    ...
    # d_mean_optim, d_mean_scheduler = optimizer.get_optimizer(args, d_mean_encoders)
    ...
    while i < args.max_iter:
        ...
        for dataset, val_loader in extra_val_loaders.items():
            ...
            with torch.no_grad():
                ...
                # if args.local_rank == 0:
                # if mean_iu > best_records[dataset]['best_mean_iu']:
                #     best_records[dataset]['best_mean_iu'] = mean_iu
                #     best_records[dataset]['best_epoch'] = epoch
                eval_res = {
                        dataset:
                        {'cur_miou': mean_iu,
                         'epoch': epoch,}}
...
def train(train_loader, net, optim, curr_epoch, writer, scheduler, max_iter):
    ...
    for i, data in enumerate(train_loader):
        ...
        for di, ingredients in enumerate(zip(inputs, gts, aux_gts)):
            ...
            #         print("Extra validating... This won't save pth file")
            #         with torch.no_grad():
            #             mean_iu = validate(val_loader, dataset, net, criterion_val, optim, scheduler, curr_epoch, writer, curr_iter, save_pth=False)
            #             # if args.local_rank == 0:
            #             # if mean_iu > best_records[dataset]['best_mean_iu']:
            #             #     best_records[dataset]['best_mean_iu'] = mean_iu
            #             #     best_records[dataset]['best_epoch'] = epoch
