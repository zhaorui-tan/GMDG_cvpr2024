...
parser.add_argument('--te_idx',                    type=int, help='leave out domain for testing ', default=0)
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        for k in eval_sample_batched:
            with torch.no_grad():
                ...
                image = torch.autograd.Variable(image.cuda(gpu, non_blocking=True))
                gt_depth = depth
            ...
            if args.garg_crop or args.eigen_crop:
                gt_height, gt_width = gt_depth.shape[-2], gt_depth.shape[-1]
...
def main_worker(gpu, ngpus_per_node, args, te_idx):
    ...
    model = VADepthNet(pretrained=args.pretrain,
                       ...
                       img_size=(args.input_height, args.input_width), args= args)
    model.train()
    ...
    dataloader = NewDataLoader(args, 'train', te_idx=te_idx)
    dataloader_eval = NewDataLoader(args, 'online_eval',te_idx=te_idx)
    env_log = f'train_env:{dataloader.train_env}, test_env: {dataloader.test_env} '
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            for k in sample_batched:
                s_b = sample_batched[k]
                images.append(s_b['image'])
                depths.append(s_b['depth'])
            images = torch.cat(images, dim=0)
            depths = torch.cat(depths, dim=0)

            image = torch.autograd.Variable(images.cuda(args.gpu, non_blocking=True))
            depth_gt = torch.autograd.Variable(depths.cuda(args.gpu, non_blocking=True))


            depth_est, loss = model(image, depth_gt, optimizer=optimizer,optimizer_y=optimizer_y, y_mapping= y_mapping, args= args)
...
def main():
    ...
    te_idx = args.te_idx
    args.log_directory = f'models_o_y_xs_{testing_envs[te_idx]}/'
    print(args.log_directory)
    command = 'mkdir '+ args.log_directory
    os.system(command)

    args.dist_url = f'tcp://127.0.0.1:200{te_idx}'
    print(args.dist_url)
    ...
    if args.multiprocessing_distributed:
        ...
        mp.spawn(main_worker, nprocs=ngpus_per_node, args=(ngpus_per_node, args, te_idx))
    else:
        main_worker(args.gpu, ngpus_per_node, args, te_idx)
